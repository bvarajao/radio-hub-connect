            ...
            {list.map((r) => (
              <article key={r.id} className="surface-panel p-4">
                ...
                <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-muted-foreground">
                  <span>Retirada: {fmt(r.pickup_at)}</span>
                  <span>Devolução: {fmt(r.due_at)}</span>
                </div>
                <div className="mt-3 flex items-center justify-between border-t border-border pt-3">
                  <span className="text-xs text-muted-foreground">
                    {r.rental_radios?.length ?? 0} rádios
                  </span>
                ...
                {["active", "late"].includes(r.status) && (
                  <Button asChild variant="soft" size="sm" className="mt-3 w-full">
                    <Link to="/devolucao/$rentalId" params={{ rentalId: r.code }}>
