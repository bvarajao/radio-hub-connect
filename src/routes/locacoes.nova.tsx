...
function NewRental() {
  ...
  const available = useMemo(
    () =>
      radios.filter(
        (r) => !["maintenance", "lost", "inactive"].includes(r.status) && !blocked.has(r.id),
      ),
    [radios, blocked],
  );
  ...
  async function save() {
    ...
    try {
      const normalizedPayment =
        total > 0 && deposit >= total ? "paid" : deposit > 0 ? "partial" : paymentStatus;
  ...
  return (
    <AppShell title="Nova Locação">
      ...
      <PageHeader
        title="Nova locação"
        subtitle="Cliente, período, equipamentos e financeiro em um único fluxo"
      />
        ...
        <div className="surface-panel space-y-5 p-4 lg:p-6">
              ...
              <div>
                <div className="flex items-center justify-between">
                  <h3 className="font-display text-base font-bold">
                    Rádios disponíveis no período
                  </h3>
                  <Badge tone="success">
                    {loadingAvailability ? "Verificando..." : `${available.length} livres`}
                  </Badge>
                </div>
                <p className="mt-1 text-xs text-muted-foreground">
                  A disponibilidade considera outras locações nas datas escolhidas.
                </p>
                  ...
                  {available.map((r) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="mt-1 inline-block rounded-full bg-secondary px-2 py-0.5 text-[0.65rem] font-bold">
                          {r.band}
                        </span>
                      </button>
                    );
                ...
                {!available.length && (
                  <p className="mt-4 text-sm text-muted-foreground">
                    Não há rádios livres para este período. Ajuste as datas ou libere equipamentos.
                  </p>
                )}
              </div>
                ...
                <div className="mt-3 space-y-2">
                  ...
                  {!accessories.length && (
                    <p className="text-sm text-muted-foreground">
                      Nenhum acessório cadastrado. Você pode cadastrá-los em Configurações.
                    </p>
                  )}
                </div>
              </div>
          ...
          {step === 2 && (
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-1.5">
                <Label>Entrada já recebida</Label>
                <Input
                  type="number"
                  min="0"
                  max={total || undefined}
          ...
          <div className="flex justify-between gap-2 border-t border-border pt-4">
            ...
            {step < 2 ? (
              ...
              <Button variant="hero" onClick={save} disabled={busy || !selected.length}>
                {busy ? "Criando..." : "Confirmar locação"} <Check className="h-4 w-4" />
              </Button>
            )}
          </div>
        </div>
        ...
        <aside className="surface-panel h-fit p-4 lg:sticky lg:top-24">
          ...
          <div className="mt-3 space-y-2 text-sm">
            ...
            <Summary
              ...
              value={
                pickup && due
                  ? `${new Date(pickup).toLocaleDateString("pt-BR")} → ${new Date(due).toLocaleDateString("pt-BR")}`
                  : "—"
              }
            />
            ...
            <Summary
              label="Faixas"
              value={[...new Set(selectedRadios.map((r) => r.band))].join(" / ") || "—"}
            />
            <Summary
              label="Acessórios"
              value={String(Object.values(acc).reduce((s, q) => s + q, 0))}
            />
          </div>
          <div className="mt-4 space-y-2 border-t border-border pt-4 text-sm">
            ...
            {deposit > 0 && (
              <p className="rounded-lg bg-success/10 p-2 text-xs text-success">
                Entrada recebida: {brlExact(Math.min(deposit, total))} · saldo:{" "}
                {brlExact(Math.max(total - deposit, 0))}
              </p>
            )}
          </div>
        </aside>
