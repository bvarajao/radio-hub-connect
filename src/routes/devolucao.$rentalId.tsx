...
import { getRentalByCode, saveReturn, type DbRental } from "@/lib/live-data";
...
function ReturnCheck() {
  ...
  const [rental, setRental] = useState<DbRental | null>(null);
  const [loading, setLoading] = useState(true);
  const [states, setStates] = useState<Record<string, Estado>>({});
  const [notes, setNotes] = useState("");
  const [busy, setBusy] = useState(false);
  useEffect(() => {
    getRentalByCode(rentalId)
      .then(setRental)
  ...
  async function finish() {
    ...
    setBusy(true);
    try {
      await saveReturn(rental!, states, notes);
    ...
    <AppShell title="Conferência">
      ...
      <div className="surface-panel grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3 p-4">
        <div>
          <p className="text-sm font-semibold">
            {checked} de {radios.length} equipamentos conferidos
          </p>
      ...
      <div className="surface-panel p-4">
        ...
        <Textarea
          ...
          placeholder="Avarias, acessórios faltantes ou observações gerais"
        />
      </div>
