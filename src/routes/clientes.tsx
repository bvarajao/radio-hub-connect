...
import { createClient, listClients, type DbClient } from "@/lib/live-data";
...
function ClientsPage() {
  ...
  const list = items.filter((c) =>
    `${c.name} ${c.document || ""} ${c.contact_name || ""}`
      .toLowerCase()
  ...
  return (
    <AppShell title="Clientes">
      <PageHeader
        ...
        actions={<NewClientDialog open={open} setOpen={setOpen} onSaved={load} />}
      />
      ...
      {list.length === 0 ? (
        <EmptyState
          ...
          description="Cadastre o primeiro cliente para começar as locações."
          action={
      ...
      <DialogContent>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <Field
            ...
            wide
          />
          <Field label="CPF / CNPJ" value={f.document} set={(v) => setF({ ...f, document: v })} />
          <Field label="Telefone" value={f.phone} set={(v) => setF({ ...f, phone: v })} />
          <Field label="E-mail" value={f.email} set={(v) => setF({ ...f, email: v })} wide />
          <Field
            label="Responsável"
