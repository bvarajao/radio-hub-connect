...
function MaintenancePage() {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((m) => {
      ...
      const matchesStatus =
        filter === "all" ||
        (filter === "open" ? !["completed", "cancelled"].includes(m.status) : m.status === filter);
      ...
      <div className="surface-panel grid gap-3 p-4 md:grid-cols-[1fr_220px]">
        <div className="relative">
          ...
          <Input
            ...
            placeholder="Buscar rádio, problema ou técnico"
            className="h-11 pl-9"
          />
        </div>
              ...
              <article key={m.id} className="surface-panel p-4">
                ...
                <div className="mt-3 flex flex-wrap gap-1 border-t border-border pt-3">
                  ...
                  {!["completed", "cancelled"].includes(m.status) && (
                    <Button variant="soft" size="sm" onClick={() => void complete(m)}>
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>{item ? "Editar manutenção" : "Nova ordem de manutenção"}</DialogTitle>
        </DialogHeader>
        <div className="grid gap-3 sm:grid-cols-2">
          <div className="space-y-1.5 sm:col-span-2">
            ...
            <Select
              ...
              onValueChange={(v) => setF({ ...f, radio_id: v })}
            >
              <SelectTrigger className="h-11">
              ...
              <SelectContent>
                {radios
                  .filter((r) =>
                    item
                      ? r.id === item.radio_id
                      : !["rented", "reserved", "lost", "inactive"].includes(r.status),
                  )
                  .map((r) => (
