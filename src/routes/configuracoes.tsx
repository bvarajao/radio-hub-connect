        ...
        <TabsContent value="usuarios" className="mt-4">
          <div className="surface-panel p-5">
            ...
            <p className="mt-4 text-xs text-muted-foreground">
              Convites para outros usuários serão adicionados em uma próxima etapa; o banco já
              suporta os perfis owner, admin, operação e financeiro.
            </p>
          </div>
        </TabsContent>
        <TabsContent value="acessorios" className="mt-4">
          <div className="surface-panel overflow-hidden">
            <div className="flex items-center justify-between gap-3 border-b border-border p-4">
              ...
              <NewAccessory open={open} setOpen={setOpen} onSaved={load} />
            </div>
            <ul className="divide-y divide-border">
              {accessories.map((a) => (
                ...
                >
                  ...
                  <span className="font-semibold">{brlExact(Number(a.unit_cost || 0))}</span>
                </li>
              ))}
              {!accessories.length && (
...
function NewAccessory({
  ...
  async function save() {
    ...
    try {
      ...
      setOpen(false);
      setF({ name: "", category: "other", stock_total: "0", unit_cost: "0", notes: "" });
      onSaved();
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "Erro ao salvar");
    } finally {
