...
import { listFinance, listRadios, listRentals } from "@/lib/live-data";
...
function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([listRadios(), listRentals(), listFinance()])
      .then(([a, b, c]) => {
        setRadios(a);
        setRentals(b);
        setFinance(c);
  ...
  const m = useMemo(
    () => ({
      ...
      reservados: radios.filter((r) => r.status === "reserved").length,
      ativas: rentals.filter((r) => ["active", "late"].includes(r.status)).length,
      atrasadas: rentals.filter(
        (r) =>
          r.status === "late" ||
          (["active", "reserved"].includes(r.status) && new Date(r.due_at) < now),
      ).length,
      faturamento: finance
        .filter((f) => f.type === "income" && new Date(f.created_at) >= monthStart)
        .reduce((s, f) => s + Number(f.amount), 0),
      areceber: finance
        .filter((f) => f.type === "income" && ["pending", "overdue"].includes(f.status))
        .reduce((s, f) => s + Number(f.amount), 0),
    }),
    [radios, rentals, finance],
  );
  return (
    <AppShell title="Dashboard">
      <PageHeader
        title="Visão geral da operação"
        subtitle={loading ? "Carregando dados..." : "Dados em tempo real do Supabase"}
        actions={
      ...
      <section className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <StatCard label="Locados" value={String(m.locados)} icon={CalendarClock} tone="brand" />
        <StatCard
          label="Disponíveis"
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="surface-panel p-5">
          <h2 className="font-display text-base font-bold">Estoque</h2>
          <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
            <Mini label="Disponíveis" value={m.disponiveis} />
            <Mini label="Locados" value={m.locados} />
            <Mini label="Reservados" value={m.reservados} />
