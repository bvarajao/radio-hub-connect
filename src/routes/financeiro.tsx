...
function FinancePage() {
  const [items, setItems] = useState<DbFinance[]>([]);
  const [clients, setClients] = useState<DbClient[]>([]);
  const [open, setOpen] = useState(false);
  const load = () =>
    Promise.all([listFinance(), listClients()])
      .then(([f, c]) => {
        setItems(f);
        setClients(c);
      })
  ...
  return (
    <AppShell title="Financeiro">
      <PageHeader
        ...
        actions={<NewFinance open={open} setOpen={setOpen} clients={clients} onSaved={load} />}
      />
...
function NewFinance({
  ...
  async function save() {
    if (!f.description || Number(f.amount) <= 0) {
