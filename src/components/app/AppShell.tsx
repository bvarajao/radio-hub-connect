...
import { ensureOrganization, getCurrentUser, getSession, signOut } from "@/lib/supabase-rest";
const nav = [
    ...
    <nav className="flex flex-col gap-1">
      {nav.map(({ to, label, icon: Icon }) => (
        <Link
          ...
          to={to}
          onClick={onNavigate}
          className="group flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-sidebar-foreground/75 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground data-[status=active]:bg-sidebar-accent data-[status=active]:text-sidebar-accent-foreground"
        >
    ...
    <div className="gradient-night flex h-full flex-col gap-6 p-4">
      ...
      <div className="space-y-3 border-t border-sidebar-border pt-4">
        ...
        <div className="flex items-center gap-3 rounded-lg px-2 py-1.5">
          <span className="grid h-9 w-9 place-items-center rounded-full bg-sidebar-accent text-xs font-bold text-sidebar-accent-foreground">
            PP
          </span>
          <div className="min-w-0 flex-1">
            ...
            <p className="text-xs text-sidebar-foreground/55">Papo de Produtor</p>
          </div>
          <button
            onClick={() => {
              signOut();
              navg({ to: "/", replace: true });
            }}
    ...
    return (
      <div className="grid min-h-screen place-items-center bg-background">
        <div className="text-center">
          <Logo />
          <p className="mt-4 text-sm text-muted-foreground">Carregando sua operação...</p>
        </div>
      </div>
    );
  return (
    <div className="min-h-screen bg-background lg:flex">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-30 grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 border-b border-border bg-background/85 px-4 py-3 backdrop-blur-md lg:px-8">
          ...
          <p className="truncate font-display text-sm font-bold lg:text-base">
            {title ?? "Painel"}
          </p>
        ...
        <nav className="fixed inset-x-0 bottom-0 z-30 grid grid-cols-5 border-t border-border bg-card/95 backdrop-blur-md lg:hidden">
          {mobileNav.map(({ to, label, icon: Icon }) => (
            <Link
              ...
              to={to}
              className={cn(
                "flex flex-col items-center gap-1 py-2.5 text-[0.65rem] font-medium text-muted-foreground data-[status=active]:text-accent-foreground",
              )}
            >
